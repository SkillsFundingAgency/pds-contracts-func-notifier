...
namespace Pds.FundingClaims.Notifications.Services.Tests.Unit.FundingClaims
{
    [TestClass, TestCategory("Unit")]
    public class FundingClaimSignedEmailServiceTests
    {
        ...
        private readonly Mock<ILoggerAdapter<FundingClaimSignedEmailService>> _mockLogger;
        ...
        public FundingClaimSignedEmailServiceTests()
        {
            _mockNotificationEmailQueueService = new Mock<INotificationEmailQueueService>(MockBehavior.Strict);
            _mockLogger = new Mock<ILoggerAdapter<FundingClaimSignedEmailService>>(MockBehavior.Strict);
            ...
            _fundingClaimSignedEmailService = new FundingClaimSignedEmailService(
        ...
        {
            // Arrange
            var serviceBusMessage = new FundingClaimSignedEmailMessage()
            {
                FundingClaimId = 1
            };
            ...
            // Act
            Func<Task> actual = async () => { await _fundingClaimSignedEmailService.Process(serviceBusMessage); };
        ...
        public async Task Process_WithValidFundingClaimAndFailedDSIApiCall_ThrowsException()
        {
            ...
            var expectedMessage = GetNotificationMessageMock(mockFundingClaim.Title, mockFundingClaim.SignedOn, mockFundingClaim.SignedByDisplayName, Constants.MessageType_FundingClaimSignedEmail);
        ...
        [TestMethod]
        public async Task Process_WithValidFundingClaimAndInvalidRoles_DoesNotSendFundingClaimSignedEmail()
        {
            // Arrange
            var serviceBusMessage = new FundingClaimSignedEmailMessage()
            {
            ...
            var expectedError = $"Funding id [{mockFundingClaim.Id}], {nameof(FundingClaimSignedEmailMessage)} processed and no users found with roles [ViewFundingClaimsAndReconciliationStatements, SignFundingClaims] for organisation [{mockFundingClaim.Ukprn}]";

            // Act
            await _fundingClaimSignedEmailService.Process(serviceBusMessage);

            // Assert
            _mockHttp.VerifyNoOutstandingRequest();
            _mockDfESignInPublicApi.Verify(api => api.GetUserContactsForOrganisation(It.IsAny<int>(), It.IsAny<string[]>()), Times.Once);
            _mockLogger.Verify(logger => logger.LogError(expectedError), Times.Once);
            _mockAuditService.Verify(audit => audit.AuditAsync(It.Is<Audit.Api.Client.Models.Audit>(x => x.Message == $"{nameof(FundingClaimSignedEmailMessage)} processed and no users found with roles [ViewFundingClaimsAndReconciliationStatements, SignFundingClaims] for organisation [{mockFundingClaim.Ukprn}]")), Times.Once);
        }

        [TestMethod]
        public async Task Process_WithValidFundingClaimAndRequiredUserRoles_SuccessfullySendsFundingClaimSignedEmail()
        {
            // Arrange
            var serviceBusMessage = new FundingClaimSignedEmailMessage()
            {
            ...
            var expectedAuditMessage = "FundingClaimSignedEmailMessage processed and published to SharedEmailprocessorQueue.";
        ...
        [TestMethod]
        public async Task Process_WithValidFundingClaimRequiredUserRolesAndNoSignedOnDateAvailable_ThrowsException()
        {
            // Arrange
            var serviceBusMessage = new FundingClaimSignedEmailMessage()
            {
        ...
        private static NotificationMessage GetNotificationMessageMock(string documentTitle, DateTime? signedOn, string signedByDisplayName, string messageType)
        {
